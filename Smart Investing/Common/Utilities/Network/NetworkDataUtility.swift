...
class NetworkDataUtility : NSObject, URLDataFetching {
    ...
    private var fileDataUtility = FileDataUtility()
    ...
    func fetchDataWithURL(url: URL, completionHandler: @escaping dataHandler) {
        // Use File Utility to get local file data
        if url.isFileURL {
            if let data = try? fileDataUtility.dataFromJSONFile(url: url) {
        ...
        // Non-file based, use network to get remote data
        var urlRequest = URLRequest(url: url,
                                    cachePolicy: .reloadIgnoringLocalCacheData,
                                    timeoutInterval: Intervals.timeoutInterval.rawValue)
